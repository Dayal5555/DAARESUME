...
export const extractResumeHTML = (): string => {
  ...
  if (!resumeContent) {
    ...
    if (previewContainer) {
      const templateContent = previewContainer.querySelector(
        '[class*="resume-template"], [id*="resume"], [class*="template"]'
      );
      if (templateContent) {
  ...
  if (!resumeContent) {
    alert(
      'Resume content not found! Please make sure the template is properly loaded.'
    );
    return '';
  }
  ...
  const nameElement = resumeContent.querySelector(
    'p.text-\\[\\#101418\\].text-lg.font-bold, h1, h2, .name, [class*="name"]'
  );
